...
def main():
    if len(sys.argv) != 2:
        print("Usage: python show_fu_domain_results.py <slurm_logs/fu_domain>")
        sys.exit(1)
    ...
    files = sorted(log_dir.glob("output_fu_domain_*.txt"))

    if not files:
        print(f"No output_fu_domain_*.txt files found in: {log_dir}")
        sys.exit(1)
