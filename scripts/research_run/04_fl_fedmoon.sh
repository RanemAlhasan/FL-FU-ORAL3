...
#SBATCH -J fl_fedmoon_oral
#SBATCH -o output_fl_fedmoon_%j.txt
#SBATCH -e error_fl_fedmoon_%j.txt
#SBATCH -p gpu-all
#SBATCH --gres gpu:1
#SBATCH -c 8
#SBATCH --mem 32G
...
# =============================================================================
# Phase 1, Run 4/4: FedMOON, full scale
# 50 global rounds / 3 local epochs from configs/fl_fedmoon.yaml.
# =============================================================================
...
echo ""
echo "=== Phase 1: FL training (FedMOON, full scale) ==="

python3 scripts/run_fl.py \
    --config configs/fl_fedmoon.yaml \
    --set dataset_path="${DATASET_PATH}" \
    --run_id "fl_fedmoon_oral_${SLURM_JOB_ID}"

echo ""
echo "Done. run_id = fl_fedmoon_oral_${SLURM_JOB_ID}"