...
#SBATCH -J fl_fedprox_oral
#SBATCH -o output_fl_fedprox_%j.txt
#SBATCH -e error_fl_fedprox_%j.txt
#SBATCH -p gpu-all
#SBATCH --gres gpu:1
#SBATCH -c 8
#SBATCH --mem 32G
...
# =============================================================================
# Phase 1, Run 2/4: FedProx, full scale
# 50 global rounds / 3 local epochs from configs/fl_fedprox.yaml.
# =============================================================================
...
echo ""
echo "=== Phase 1: FL training (FedProx, full scale) ==="

python3 scripts/run_fl.py \
    --config configs/fl_fedprox.yaml \
    --set dataset_path="${DATASET_PATH}" \
    --run_id "fl_fedprox_oral_${SLURM_JOB_ID}"

echo ""
echo "Done. run_id = fl_fedprox_oral_${SLURM_JOB_ID}"