#!/bin/bash -l
#SBATCH -J ret_india_fbn_std
#SBATCH -o output_retrain_india_fedbn_stdce_%j.txt
#SBATCH -e error_retrain_india_fedbn_stdce_%j.txt
#SBATCH -p gpu-all
#SBATCH --gres gpu:1
#SBATCH -c 16
#SBATCH --mem=64G
...
echo "Source FL run: fl_fedbn_oral_344913"
echo "Forget client: India_Dataset"
nvidia-smi
...
echo ""
echo "=== Exact retraining: FedBN + Standard CE, forget India ==="

python3 scripts/run_retrain.py \
  --config configs/retrain_fedbn_stdce.yaml \
  ...
  --run_id "retrain_india_fedbn_stdce_oral_${SLURM_JOB_ID}"

echo ""
echo "Done. run_id = retrain_india_fedbn_stdce_oral_${SLURM_JOB_ID}"